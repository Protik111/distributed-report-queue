...
app.get("/api/stats", async (_req, res) => {
  try {
    const queue = new Queue(REPORT_QUEUE, { connection: redis });
...
app.get("/api/jobs/:jobId", async (req, res) => {
  try {
    ...
    const resultRaw = await redis.get(`job:result:${jobId}`);
    ...
    res.json({
      id: jobId,
      status: job ? await job.getState() : (result?.success ? "completed" : "unknown"),
  ...
  try {
    ...
    const jobs = await Promise.all(
      failedJobs.map(async (job) => ({
        ...
        attempts: job.attemptsMade,
        errorDetails: JSON.parse((await redis.get(`job:error:${job.id}`)) || "null"),
      }))
    );
